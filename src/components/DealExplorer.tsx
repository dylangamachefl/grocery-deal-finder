...
const DealExplorer: React.FC<DealExplorerProps> = ({ categories, savedDeals, onAddToShoppingList, isLoading }) => {
  ...
  // Auto-expand categories with high-confidence matches (>= 0.85)
  React.useEffect(() => {
    if (categories.length > 0) {
      const highConfidenceCategories = categories
        .filter(cat =>
          cat.items.some(item => (item.confidence || 0) >= 0.85)
        )
        .map(cat => cat.category);

      if (highConfidenceCategories.length > 0) {
        setExpandedCategories(new Set(highConfidenceCategories));
      }
    }
  }, [categories]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 pb-4 border-b border-slate-100">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all duration-200 border ${selectedStore === 'All'
            ...
            {uniqueStores.map((store) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all duration-200 border ${selectedStore === store
                    ? 'bg-purple-600 border-purple-600 text-white shadow-md shadow-purple-200'
                    : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-purple-300'
                  }`}
              >
                    ...
                    <div key={deal.id} className="p-4 hover:bg-slate-50 transition-colors flex flex-col sm:flex-row justify-between gap-4">
                      ...
                      <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-3 min-w-[100px] border-t sm:border-t-0 sm:border-l border-slate-100 pt-3 sm:pt-0 sm:pl-4">
                        ...
                        <button
                          ...
                          className={`text-sm font-medium px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all w-full sm:w-auto justify-center ${isSaved(deal)
